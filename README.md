# Data for: POLITICIANS DANCING ON TABLES

This repository contains the experimental stimuli, model response annotations, and analysis files
used in the bachelor thesis:

"POLITICIANS DANCING ON TABLES: How Large Language Models handle false presuppositions"

## Contents

- /data
  - annotations_chatgpt51.csv: Model responses and annotations (ChatGPT-5)
  - annotations_gemini25.csv: Model responses and annotations (Gemini 2.5 Flash)
  - annotations_deepseekv32.csv: Model responses and annotations (DeepSeek v3.2)

- /analysis
  - data_allmodels.jasp: JASP file containing all cross model analyses
  - data_chatgpt51.jasp: JASP file containing all statistical analyses or Chat GPT5.1
  - data_gemini25.jasp: JASP file containing all statistical analyses for Gemini 2.5 Flash
  - data_deepseekv32.JASP: JASP file containing all statistical analyses for DeepSeek v3.2

## Notes

All stimuli were tested once per model.
The dataset is provided for transparency and reproducibility.

Credit and blame: Liam Hultelid
